...
def get_individual_metrics(
    ...
    user_unsafe_sites = len(unsafe_sites.get(email, []))
    user_reused_pwds = len(reuse_passwords.get(email, []))
    user_file_downloads = len(file_downloads.get(email, []))
    user_malware_downloads = len(malware_download.get(email, []))
    user_vulnerable_pwds = len(vulnerable_passwords.get(email, []))
